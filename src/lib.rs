        ...
        fn provide_websocket_inner(url: &str) -> Result<Option<WebSocket>, JsValue> {
            ...
            let callback = Closure::wrap(Box::new(move |event: MessageEvent| {
                ...
                if let Ok(update_signal) = serde_json::from_str::<ServerSignalUpdate>(&ws_string) {
                    ...
                    // Get an owned copy of the signal (if present) without holding the lock
                    let maybe_signal = {
                    ...
                    if let Some(signal) = maybe_signal {
                        // Apply any queued patches
                        if let Some(delayed_patches) = {
                            let mut dm = delayed_updates.lock().unwrap();
                            dm.remove(name)
                        } {
                            signal.update(|doc| {
                        ...
                        let mut dm = delayed_updates.lock().unwrap();
                        dm.entry(name.clone()).or_default().push(update_signal.patch.clone());
                    }
                }
            }) as Box<dyn FnMut(_)>);
